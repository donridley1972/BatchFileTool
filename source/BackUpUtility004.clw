   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('BACKUPUTILITY004.INC'),ONCE        !Local module procedure declarations
                     END
...
window               WINDOW('Runtime File Manager'),AT(,,321,233),FONT('Tahoma',8,,FONT:regular),DOUBLE,AUTO,CENTER, |
  ICON('appicon.ico'),GRAY,SYSTEM,WALLPAPER('gradient(1).png'),IMM
...
BuildListQueue  Routine
  clear(ListQueue)
  lq:filename = 'BackUpFiles'
  lq:filedesc = 'BackUpFiles'
  lq:FileRecords  = '?'
  lq:FileHandle &= BackUpFiles
...
ManageFile ROUTINE
  ...
  of upper('BackUpFiles')
    LocFMx:WorkFile &= BackUpFiles
    LocFMx:WorkFilePrefix = 'Bac'
  of upper('Projects')
